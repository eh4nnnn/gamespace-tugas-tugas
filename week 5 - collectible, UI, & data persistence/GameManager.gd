...
func reset_game():
	save_game = null # Mengosongkan data di memory
	var path = "user://savegame.tres" # Sesuaikan dengan nama file save kamu
	if FileAccess.file_exists(path):
		DirAccess.remove_absolute(path) # Menghapus file di komputer
	get_tree().reload_current_scene() # Mengulang scene agar musuh muncul lagi
